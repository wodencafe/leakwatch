...
@LeakTracked(tags = {"temp-file", "fallback-cleanup"})
@FallbackCleanup(action = DeletePathFallbackCleanup.class)
public final class TrackedTempFile implements AutoCloseable, FallbackCleanupStateProvider<Path> {
    private final Path path;

    public TrackedTempFile(String prefix) throws IOException {
